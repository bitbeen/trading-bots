...
const main = async() =>{
    ...
    var data = await last10Minutes(pool).then(async data =>{
    ...
    console.log(`Predicted potential price change in 10 mins ${round(result.predictedChange)}`);
    console.log(`Predicted potential price change% in 10 mins: ${round(result.predictedChangePercentage)}%`);
...
function calculateEMA(data, smoothingFactor) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const alpha = 1 - Math.exp(-timeDifference / (10 * 60 * 1000 * smoothingFactor)); // Convert smoothingFactor to 10-minute intervals
  ...
  // Calculate the EMA for the historical data within the past 10 minutes
    const lastTimestamp = clonedData[clonedData.length - 1].time;
    const tenMinutesAgo = lastTimestamp - (10 * 60 * 1000);
    const dataWithinTenMinutes = clonedData.filter(data => data.time >= tenMinutesAgo);
    const ema = calculateEMA(dataWithinTenMinutes, smoothingFactor);
...
const last10Minutes = async() =>{
    let url = `https://api.geckoterminal.com/api/v2/networks/polygon_pos/pools//${pool}/ohlcv/minute?aggregate=1&limit=10`
    let data 
    await axios.get(url)
        .then((result) =>{
